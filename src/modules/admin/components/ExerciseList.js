...
const ExerciseList = React.forwardRef(
  ({ className, cardImage, bgPopular, ...props }, ref) => {
    ...
    const columns = [
      ...
      {
        name: "Mechanics",
        selector: (row) => row.mechanics,
        cell: (row) => (
          <div className="py-1  text-sm w-full ">{row.mechanics}</div>
        ),
      },
    ];
      ...
      <div>
        ...
        <DataTable
          ...
          subHeaderComponent={
            <input
              type="text"
              className={`font-custom  block w-full md:w-full lg:w-1/2 pl-0 md:pl-0 lg:pl-2 rounded-md border-0 py-1.5 text-text shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:outline-none focus:ring-inset focus:ring-active hover:ring-active sm:text-sm sm:leading-6`}
